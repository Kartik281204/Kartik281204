...
Run by .github/workflows/update-profile-art.yml, right after fetch_contributions.py.
This is the single source of truth for the heatmap -- no third-party API in the
loop, just GitHub's own public contributions page via fetch_contributions.py.
"""
import datetime
import json
import os
...
""".strip()
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">{GH_USER.lower()}@github: ~/contributions --graph</text>')

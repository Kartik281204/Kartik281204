...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "kartik-ascii.svg")

GH_USER = os.environ.get("GH_PROFILE_USER", "Kartik281204")
DISPLAY_NAME = os.environ.get("GH_PROFILE_NAME", "Kartik Singh")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">{html.escape(GH_USER.lower())}@github: ~$ ./portrait.sh</text>')
...
whoami_prefix = f'{html.escape(GH_USER.lower())}@github:~$ whoami '
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{whoami_prefix}<tspan fill="{INK}">{html.escape(DISPLAY_NAME)}</tspan></text>')
cursor_x = PAD + len(whoami_prefix) * 7.1 + len(DISPLAY_NAME) * 7.1

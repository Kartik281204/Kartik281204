...
GH_USER = os.environ.get("GH_PROFILE_USER", "Kartik281204")
...
ROWS = [
    ("host",),
    ("kv", "Now", "AI Product & Biz Analyst @ Polluxa"),
    ("kv", "Prev", "Business Analyst @ Maruti Suzuki"),
    ("kv", "Also", "AI Product & Data Sci @ Gravity Eng."),
    ("kv", "Edu", "B.Tech CSE - Data Sci, UPES '27"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Languages", "Python, SQL, TypeScript, JavaScript"),
    ("kv", "AI / ML", "Scikit-learn, XGBoost, LangChain, Claude API"),
    ("kv", "Data / BI", "Pandas, PostgreSQL, Power BI, Tableau"),
    ("kv", "Backend", "FastAPI, Node.js, Next.js, Docker, AWS"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Production ML pipeline on 90 yrs of S&P 500 data"),
    ("bul", "Built Sightline — grounded AI analytics copilot"),
    ("bul", "Insights presented at GM & founder level, 4 internships"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">{esc(GH_USER.lower())}@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">{esc(GH_USER.lower())}</tspan><tspan fill="{MUTED}">@</tspan>'
